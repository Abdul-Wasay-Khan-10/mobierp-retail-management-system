...
CREATE TABLE products (
  ...
  track_individually BOOLEAN DEFAULT FALSE, -- For future IMEI/Serial tracking
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE sales (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  sale_number VARCHAR(50) NOT NULL UNIQUE, -- Human-readable sale number
...
CREATE TABLE sale_items (
  ...
  cost_price DECIMAL(10, 2) NOT NULL CHECK (cost_price >= 0), -- Store cost at time of sale
...
CREATE TABLE sku_counters (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  date_key VARCHAR(20) NOT NULL UNIQUE, -- Format: DDMMYYYY
...
-- Policies (Allow all for authenticated users - adjust based on your auth strategy)
...
CREATE POLICY "Allow all for authenticated users" ON inventory_units FOR ALL USING (true);
CREATE POLICY "Allow all for authenticated users" ON sku_counters FOR ALL USING (true);
...
-- Insert default admin user (password: hamza123 - hash this in production!)
-- Note: You should hash passwords properly in production
INSERT INTO users (username, password_hash, name, role) VALUES
  ('hamza', '$2a$10$example_hash_replace_this', 'Hamza (Admin)', 'OWNER'),
  ('staff', '$2a$10$example_hash_replace_this', 'Store Staff', 'STAFF');
...
GROUP BY s.id, s.sale_number, s.total_amount, s.created_at, u.name, u.username;
...
COMMENT ON TABLE products IS 'Stores all product inventory items';
COMMENT ON TABLE sales IS 'Master table for all sales transactions';
COMMENT ON TABLE sale_items IS 'Line items for each sale transaction';
COMMENT ON TABLE inventory_units IS 'Individual tracking for high-value items (IMEI, Serial numbers)';
COMMENT ON COLUMN products.track_individually IS 'If true, requires serial/IMEI tracking in inventory_units';
COMMENT ON FUNCTION generate_sku IS 'Auto-generates SKU in format: CAT-BRAND-DDMMYYYY-XXXX';
